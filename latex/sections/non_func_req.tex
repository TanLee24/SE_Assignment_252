...
\subsection{Nhóm Điều hướng \& Giám sát IoT}
...
\subsection{Nhóm Tính phí \& Thanh toán (BKPay)}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Kết nối giữa IoT-SPMS và BKPay phải sử dụng chữ ký điện tử (Digital Signature/HMAC) để chống giả mạo gói tin thanh toán.
        \item Không lưu trữ bất kỳ thông tin nhạy cảm nào về thẻ ngân hàng/tài khoản ví điện tử của người dùng trên IoT-SPMS (hoàn toàn ủy thác cho BKPay).
        \item Endpoint webhook/callback phải có cơ chế idempotency (chống xử lý trùng) theo cặp (invoiceId, txId) để tránh cập nhật trạng thái sai khi BKPay retry.
    \end{itemize}
    ...
    \item Tính sẵn sàng và khôi phục (Availability \& Recovery):
    \begin{itemize}
        \item Dịch vụ tính phí/thanh toán phải có cơ chế retry khi gọi API BKPay (giới hạn số lần, có backoff) và phải timeout rõ ràng để tránh treo giao dịch.
        \item CSDL hóa đơn/giao dịch phải được sao lưu định kỳ; mục tiêu khôi phục dữ liệu sau sự cố (RPO) không quá 24 giờ đối với dữ liệu hóa đơn và log đối soát.
    \end{itemize}
\end{itemize}